...
function j(i, e) {
    ...
    return e === 'source' || e === 'global' || e === 'ignoreCase' || e === 'multiline' ? O(i) === 'RegExp' : !1
}
class P {
    constructor(e) {
    ...
    _getSlots(e) {
        ...
        const t = new Set(),
            ...
            u = RegExp.prototype
        for (; e != null && e !== r && e !== o && e !== c && e !== s && e !== n && e !== u; ) {
    ...
    _makeComponent(e, t) {
        ...
        if (e.type === 'constant' || e.type === 'collection' || e.type === 'null' || e.type === 'undefined') return e.value
        if (e.type === 'function') {
            const r = (e.members && e.members.prototype && e.members.prototype.members) || {},
                ...
                s = T(function (...u) {
                    ...
                    l.results.push(p), l.invocationCallOrder.push(c._invocationCallCounter++)
                    let m,
                        w,
                        k = !1
                    try {
                            ...
                            if (this instanceof n) {
                                o.forEach((y) => {
                                    if (r[y].type === 'function') {
                                        const A = this[y]
                                        ;(this[y] = c.generateFromMetadata(r[y])), (this[y]._protoImpl = A)
                                    }
                                })
            ...
            return (
                ...
                (n.mockResolvedValueOnce = (u) => n.mockImplementationOnce(() => Promise.resolve(u))),
                (n.mockRejectedValueOnce = (u) => n.mockImplementationOnce(() => Promise.reject(u))),
                (n.mockReturnValue = (u) => n.mockImplementation(() => u)),
                (n.mockResolvedValue = (u) => n.mockImplementation(() => Promise.resolve(u))),
                (n.mockRejectedValue = (u) => n.mockImplementation(() => Promise.reject(u))),
                (n.mockImplementationOnce = (u) => (this._ensureMockConfig(n).specificMockImpls.push(u), n)),
    ...
    _createMockFunction(e, t) {
        ...
        if (r === d) return t
        ;(V.has(r) || /^\d/.test(r)) && (r = '$' + r), I.test(r) && (r = r.replace(G, '$'))
        const s = 'return function ' + r + '() {return ' + d + '.apply(this,arguments);}' + c
    ...
    getMetadata(e, t) {
        ...
        if (!c) return null
        const s = { type: c }
        if (c === 'constant' || c === 'collection' || c === 'undefined' || c === 'null') return (s.value = e), s
        c === 'function' && ((s.name = e.name), e._isMockFunction === !0 && (s.mockImpl = e.getMockImplementation())),
            (s.refID = r.size),
            r.set(e, s.refID)
        let n = null
        return (
            c !== 'array' &&
                this._getSlots(e).forEach((u) => {
                    if (c === 'function' && e._isMockFunction === !0 && u.match(/^mock/)) return
    ...
    spyOn(e, t, r) {
        ...
        if (!this.isMockFunction(o)) {
            if (typeof o != 'function')
                throw new Error(
                    'Cannot spy the ' + t + ' property because it is not a function; ' + this._typeOf(o) + ' given instead',
                )
            const c = Object.prototype.hasOwnProperty.call(e, t)
            let s = Object.getOwnPropertyDescriptor(e, t),
                n = Object.getPrototypeOf(e)
            for (; !s && n !== null; ) (s = Object.getOwnPropertyDescriptor(n, t)), (n = Object.getPrototypeOf(n))
            let u
            if (s && s.get) {
                const l = s.get
    ...
    _spyOnProperty(e, t, r = 'get') {
        ...
        for (; !o && c !== null; ) (o = Object.getOwnPropertyDescriptor(c, t)), (c = Object.getPrototypeOf(c))
        if (!o) throw new Error(t + ' property does not exist')
        if (!o.configurable) throw new Error(t + ' is not declared configurable')
        if (!o[r]) throw new Error('Property ' + t + ' does not have access type ' + r)
        const s = o[r]
        if (!this.isMockFunction(s)) {
            if (typeof s != 'function')
                throw new Error(
                    'Cannot spy the ' + t + ' property because it is not a function; ' + this._typeOf(s) + ' given instead',
                )
...
const { addons: N } = __STORYBOOK_MODULE_PREVIEW_API__,
    { FORCE_REMOUNT: Y, STORY_RENDER_PHASE_CHANGED: B } = __STORYBOOK_MODULE_CORE_EVENTS__
var E = new v(D),
    L = E.fn.bind(E),
